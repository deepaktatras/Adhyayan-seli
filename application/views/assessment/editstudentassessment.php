...
<a href="<?php
				       ...
				       Edit Review - <?php echo $assessment['client_name']; ?>
                                       ...
                                       ?>
                                            <div style="float:right"><a href="?controller=assessment&action=uploadstudentprofile&gaid=<?php echo $gaid ?>" class="btn  btn-primary">Upload Student Profile</a></div>
                                       <?php
                                       }
                                       ?>
</h1>
...
<div>
	<div class="ylwRibbonHldr">
		...
		<div class="collapse navbar-collapse tabitemsHldr" id="tab4_Toggle">
			<ul class="yellowTab nav nav-tabs">          
				<li class="item active"><a href="#editTeacherAssessment-step1" data-toggle="tab" class="vtip" title="Edit Student Review">Step 1</a></li>
				<li class="item"><a href="#editTeacherAssessment-step2" data-toggle="tab" class="vtip" title="Manage Validators/Students" id="step2">Step 2</a></li>				
			</ul>
		</div>
	</div>
	<div class="subTabWorkspace pad26">	
		<div class="tab-content">
			...
			<form id="editStudentAssessment" method="post" onsubmit="return false;">
                            <input type="hidden" name="student_profile_form_id" value="1" />
				...
				<div class="boxBody"><input type="hidden" name="gaid" value="<?php echo $gaid; ?>">
									...
									<dl class="fldList">
										...
										<dd><div class="row"><div class="col-sm-6">
											<select class="form-control school_admin_id" autocomplete="off" name="school_admin_id" required>
												<option value=""> - Select School/ Batch Admin/Principal - </option>
												<?php
									...
									<dl class="fldList">
										<dt>Assign Diagnostic<span class="astric">*</span>:</dt>
										<dd>
										<?php
                                                                                //print_r($category_diagnostics);
										...
										?>
                                                                        ...
                                                                        <dl class="fldList">
										<dt>Assign category<span class="astric">*</span>:</dt>
										<dd>
										
											<div class="row">
												
												<div class="col-sm-6">
													<select class="form-control diagnostic_dd" autocomplete="off" name="student_review_type" >
														<option value=""> - Select category - </option>
														<?php
                                                                                                                
														foreach($StudentReviewType as $type)
                                                                                                                        if($assessment['student_review_type_id']==$type['review_student_type_id']){
                                                                                                                         echo "<option value=\"".$type['review_student_type_id']."\" selected>".$type['review_student_type']."</option>\n";   
                                                                                                                        }else{
                                                                                                                            
															echo "<option value=\"".$type['review_student_type_id']."\">".$type['review_student_type']."</option>\n";
                                                                                                                        }
														?>
													</select>
												</div>
													...
													<select class="form-control" autocomplete="off" name="student_round" >
														...
														<?php
                                                                                                                foreach ($aqsRounds as $aqsRound){
                                                                                                                    if(in_array($aqsRound['aqs_round'],$aqsRoundsUnused)){
                                                                                                                     $disabled="";   
                                                                                                                    }else{
                                                                                                                        $disabled='disabled="disabled"';  
                                                                                                                    }
                                                                                                                    if($aqsRound['aqs_round']==$teacherAssessment['student_round']){
                                                                        ...
                                                                        <!-- <dl class="fldList">
										<dt>Choose Batch No.<span class="astric">*</span>:</dt>
										<dd>
										
											<div class="row">
												
												<div class="col-sm-6">
													<select class="form-control diagnostic_dd" autocomplete="off" name="student_batch_no" >
														<option value=""> - Select Batch - </option>
														<?php
														/*foreach($StudentBatchType as $type)
                                                                                                                        if($assessment['student_review_batch_id']==$type['student_review_batch_id']){
                                                                                                                        echo "<option value=\"".$type['student_review_batch_id']."\" selected>".$type['student_review_batch']."</option>\n";   
                                                                                                                        }else{
															echo "<option value=\"".$type['student_review_batch_id']."\">".$type['student_review_batch']."</option>\n";
                                                                                                                        }
								...
								<input type="button" data-ignorerejected="0" title="Click to update review" value="Update" class="btn btn-primary vtip" id="updateStudentAssessment" autocomplete="off">
								</div>
								<?php //} ?>
								</div>
			</form>					
			...
			<div role="tabpanel" class="tab-pane fade in" id="editTeacherAssessment-step2">
				<h2>Manage Validators/Students</h2>
...
Save Validators</button>
		...
		<?php //} ?>
				...
				<?php //if($teacherAssessment['assessmentAssigned']==0){ ?>
                            <a title="Upload student list" class="btn btn-primary small-btn pull-right execUrl vtip " id="up_teacher" href="?controller=assessment&amp;action=assessorListUpload&type=student&taid=<?php echo $teacherAssessment['group_assessment_id']; ?>&amp;ispop=1">Upload List</a>
                                <?php //} ?>
			</h2>
				...
				<?php //if($teacherAssessment['assessmentAssigned']==0){ ?>
                            <a href="javascript:void(0)" class="fltdAddRow" data-attach="<?php echo $teacherAssessment['group_assessment_id']; ?>" data-type="studentForAssessment"><i class="fa fa-plus"></i></a>
                                <?php //} ?>
				<table class="table customTbl">
					<thead>
						<tr>
					...
					<tbody>
                      ...
                      <div style="padding-bottom:50px;">       
                        <?php if($teacherAssessment['assessmentAssigned']==0){ ?>
                        <button type="button" class="btn btn-primary pull-right vtip" data-ignorerejected="0"  disabled="disabled" title="Save/ Update Students" id="saveTchrsForAssessmnt">Save Student</button>
