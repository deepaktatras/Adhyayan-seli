...
class individualReport extends reportClass{
	...
	public function generateOutput($lang_id=DEFAULT_LANGUAGE){
		...
		switch($this->reportId){ 
			...
			case 2:
				...
				$diagId==1?($this->config['isCoBranded']=1 && $this->config['coBrandedImg']= ''.UPLOAD_URL_DIAGNOSTIC.'diagnostic_image_'.$diagId.'.jpg'):'';//for Don-Bosco show logo in reports
                                ...
                                return $this->generate7thKpaAqsOutput($lang_id);
				break;
                        case 13:
				$diagId = $this->getDiagnosticId();
                                $this->config['iscollegeReview']=1;    
				//$diagId==1?($this->config['iscollegeReview']=1 && $this->config['coBrandedImg']= '.'.DS.'uploads'.DS.'diagnostic'.DS.'diagnostic_image_'.$diagId.'.jpg'):'';//for Don-Bosco show logo in reports
			...
			case 3:
				...
				$diagId==1?($this->config['isCoBranded']=1 && $this->config['coBrandedImg']= ''.UPLOAD_URL_DIAGNOSTIC.'diagnostic_image_'.$diagId.'.jpg'):'';//for Don-Bosco show logo in reports
                                ...
                                return $this->generateRecommendationOutput($lang_id);
				break;
			case 9:
			...
			case 10:
                                return $this->singleStudentOutput();
				break;
                        case 7:
	...
	protected function generateAqsOutput($lang_id=DEFAULT_LANGUAGE){
		...
		/*if($this->subAssessmentType==1)
		{
			...
			$this->loadLastPage();award_scheme_id
		}*/
		...
		$this->loadAqsDataRound2($ids_allow[0]);
               ...
               // echo '<pre>'; print_r($this->aqsData);die;
...
//                $this->loadKeyQuestionsRound2('',$lang_id,$ids_allow[1]);
		...
		if($lang_id==9){
                 $this->aqsInfo='<div class="bigBold">'.$this->aqsData[1]['school_name'].',&nbsp;'.$this->aqsData[1]['school_address'].'</div>
                         <div class="reportInfo">'.$diagnosticLabels['Compilation_Scores'].'<br><span style="font-weight: bold;">Comparison between performance in Round 1 and Round 2 of AQS<br>Round 1  '.str_replace("&",$round1_date,$diagnosticLabels['Conducted_On']).'<br>Round 2 '.str_replace("&","$round2_date",$diagnosticLabels['Conducted_On']).'<br></span></div>';
                }else{
                  $this->aqsInfo='<div class="bigBold">'.$this->aqsData[1]['school_name'].',&nbsp;'.$this->aqsData[1]['school_address'].'</div>
                         <div class="reportInfo">'.$diagnosticLabels['Compilation_Scores'].'<br><span style="font-weight: bold;">Comparison between performance in Round 1 and Round 2 of AQS<br>Round 1  '.str_replace("&",$round1_date,$diagnosticLabels['Conducted_On']).'<br>Round 2 '.str_replace("&","$round2_date",$diagnosticLabels['Conducted_On']).'<br></span></div>';
		...
		$this->indexArray=array();
               ...
               // echo "<pre>";print_r($this->generateSection_ScoreCardForKPAsRound2('',$lang_id,$diagnosticLabels,0,$isCollobrativeR1,$isCollobrativeR2));die;
		...
		$this->aqsInfo='<div class="bigBold">{schoolName}, {schoolAddress}</div>
		<div class="reportInfo">A compilation of scores based on<br>College Self-Review & Evaluation (SSRE team - College Assessors)<br>And<br>College External Review & Evaluation (SERE team - Adhyayan Assessors)<br>conducted in: {conductedOn}<br>Valid until: {validTill} </div>';
	...
	protected function generate7thKpaAqsOutput($lang_id=DEFAULT_LANGUAGE){
		...
		$this->loadAqsData();
		$this->loadAwardScheme($lang_id);
		$this->loadJudgementalStatements(true,$lang_id);
		$this->loadCoreQuestions(true,$lang_id);
		$this->loadKeyQuestions(true,$lang_id);
		$this->loadKpas(true,$lang_id);
		
               
                
		/*$this->aqsInfo='<div class="bigBold">{schoolName}, {schoolAddress}</div>
		<div class="reportInfo">A compilation of scores based on<br>School Self-Review & Evaluation (SSRE team - School Assessors)<br>And<br>School External Review & Evaluation (SERE team - Adhyayan` Assessors)<br>conducted in: {conductedOn}<br>Valid until: {validTill} </div>';
	...
	protected function generateRecommendationOutput($lang_id=DEFAULT_LANGUAGE){
                $isCollobrative=$this->config['isCollobrative'];        
                $diagnosticLabels=array();
                $languageLabels = $this->getDiagnosticLabels($lang_id);
                foreach($languageLabels as $data) {
		...
		$this->loadAqsData();
		$this->loadAwardScheme($lang_id);
		$this->loadJudgementalStatements(false,$lang_id);
		$this->loadCoreQuestions(false,$lang_id);
		$this->loadKeyQuestions(false,$lang_id);
		$this->loadKpas(false,$lang_id);
		$this->loadAward($lang_id);
		$this->loadAqsTeam();
		$this->loadAssessorKeyNotes();
		$this->loadRecommendations($lang_id);
		//print_r();
                if($isCollobrative==1){
                 $this->aqsInfo='
		<div class="recomCoverBlock">
			...
			<div class="">Based on Adhyayan’s Collaborative School Review and Evaluation (CSRE)</div>
		</div>';   
                }else{
		$this->aqsInfo='
		<div class="recomCoverBlock">
			<div class="bigText">Adhyayan Quality Standard Report</div><br><br><br><br>';
                        ...
                        $this->aqsInfo.='<div class="mediumText">Review Criteria : '.$this->aqsData['review_criteria'].'</div><br><br><br>';
                        }     
			$this->aqsInfo.='<div class="mediumText">Recommendations for School Improvement following the award of </div><br><br>
			<div class="mBigText">{awardName}</div><br>
			<div class="">Valid until: {validTill}</div><br><br><br>
			<div class="mediumText">To</div><br><br><br>
			<div class="mBigText">{schoolName}, {schoolAddress}</div><br>
		...
		$this->schoolEffectivenessInApplyingSelfReview($diagnosticLabels);
		$this->assessorKeyNotes();
		$this->aSchoolImplovementJourney();
                //$this->jdGraphReport();
                //$this->jrGraphReport();
		if($isCollobrative==0){
                $this->JDistanceOnKpa($lang_id);
                }
                
		$this->JRatingOnKpa($lang_id,$diagnosticLabels);
                $this->keyForReadingRecommendationReport();
		$this->recommendationsOnKpa($lang_id,$diagnosticLabels);
		$this->generateIndexAndCover($diagnosticLabels);
...
EOD;
		...
		//$pdf->SetMargins(15, $pdf->top_margin, 15);
		$pdf->footer_text = $this->config['coverAddress'];		
		$pdf->other_footer_text = 'Student Performance Review: Recommendation Report of '.$this->teacherInfo['name']['value'].', '.$this->aqsData['school_name'].' '.$this->schoolCity.', '.$this->schoolState.', '.$this->schoolCountry.', '.date("d-m-Y").' (generated on)';//'AQS Single Teacher Recommendation Report, '.$this->teacherInfo['name']['value'].' - '.$this->aqsData['school_name'].', '.date("d-m-Y").' (generated on)';
		$pdf->footerBG = $this->config['footerBG'];
		$pdf->footerColor = $this->config['footerColor'];
		$pdf->footerHeight = $this->config['footerHeight'];
		$pdf->pageNoBarHeight = $this->config['pageNoBarHeight'];
		$pdf->SetTitle(($this->teacherInfo['name']['value']).'-Recommendation Report');
		$pdf->SetHeaderData ( '', '', 'Student Performance Review: Recommendation Report' );
		$pdf->setHeaderFont ( Array (
				PDF_FONT_NAME_MAIN,
				'',
				PDF_FONT_SIZE_MAIN
		...
		if($this->aqsData['client_id']==11)
			$firstpagehtml = '<table class="pdfHdr broad"> <tbody><tr><td class="halfSec fl" align="left"><a href=""><img src="' . SITEURL . 'public/images/logo.png"alt="" style="height:55px;"></a></td><td class="halfSec fr" align="right" ><a href=""><img src="'.SITEURL.'/public/images/shishuvan.jpg" alt=""></a></td></tr>
			<tr><td colspan="2" style="font-weight:bold;text-align:center;font-size:13px;">The Student Performance Review has been initiated and validated by '.($this->aqsData['school_name']).'</td></tr></table>';
		else		 
			$firstpagehtml = '<br/><div style="text-align:center;"><img src="' . SITEURL . 'public/images/logo.png"></div><br/>';
		$firstpagehtml .= <<<EOD
		<div style="background-color:#800000;color:#FFFFFF;text-align:center;"><br/>
		<h2 style="text-align:center">CONTINUING PROFESSIONAL DEVELOPMENT: Recommendation Report<br/></h2>
		Recommendations for professional development for<br/><br/><br/>		
		<span style="font-family:dejavuserifb;"><b>{$this->teacherInfo['name']['value']}</b></span>{$this->awardName}<br/><br/><br/>
		Valid until: $this->validDate <br/><br/><br/>
		{$this->aqsData['school_name']}<br/><br/><br/>
...
EOD;
		...
		$pdf->addPage ();
		$sectionNum = 1;
		$pdf->Bookmark ( $sectionNum.'. Student Performance Review: Process', 0, 0, '', 'B', array (
				0,
				64,
				128
		) );
		...
		$pdf->SetTextColor ( 255, 255, 255 );
		$pdf->SetFillColor ( 108, 13, 16 );
		$pdf->Cell ( 0, 8, ($sectionNum++).'. Student Performance Review: Process', 0, 1, 'C', true );
		$pdf->SetFont ( 'times', '', 10 );
		$pdf->SetTextColor ( 0, 0, 0 );
		$pdf->ln(3);
		$html = $this->getPDFreviewProcess();		
		...
		$pdf->addPage ();	
		$pdf->SetFont ( 'times', '', 12 );
		$pdf->Bookmark ( $sectionNum.'. Student Performance Review: Grade Definition', 0, 0, '', 'B', array (
				0,
				64,
				128
		) );
		...
		$pdf->SetTextColor ( 255, 255, 255 );
		$pdf->SetFillColor ( 108, 13, 16 );
		$pdf->Cell ( 0, 8, ($sectionNum++).'. Student Performance Review: Grade Definition', 0, 1, 'C', true );
		$pdf->SetFont ( 'times', '', 12 );
		$pdf->SetTextColor ( 0, 0, 0 );		
		$pdf->ln(3);
		$html = $this->getTeacherAwardDefinition();
		...
		//key question wise review
		$pdf->AddPage();
		$pdf->Bookmark ( $sectionNum.'. Student Performance Analysis: Key Question wise', 0, 0, '', 'B', array (
				0,
				64,
				128
		) );
		...
		$pdf->SetFillColor ( 108, 13, 16 );
		$pdf->Ln(3);
		$pdf->Cell ( 0, 8, ($sectionNum++).'. Student Performance Analysis: Key Question wise', 0, 1, 'C', true );
		//create table
		$html = $this->getPDFtchrPerformance();
		//echo $html;die;
		$pdf->SetTextColor ( 0, 0, 0 );
		...
		//jd
		$pdf->AddPage();
		$pdf->Bookmark ( $sectionNum.'. Student\'s effectiveness in applying the self-review diagnostic', 0, 0, '', 'B', array (
				0,
				64,
				128
		) );
		...
		$pdf->Ln(3);
		$uri = $this->createURISqJd();
		$graph = 'The following graph depicts the levels of agreements and distance between the Self-Review Rating (SRR) and External Review Rating (ERR). <br/>The expectation is that JD=0 and 1 would increase significantly when the student does its
 				self-review validation for the second time; and by the third validation, be almost 100%. In
				this manner the student would achieve the rigour and objectivity of an external
				reviewer.<br/><div style="text-align:center;font-weight:bold;"><span style="font-weight:normal;">Table '.++$this->tableNum.'</span><br/>Judgement Distance (JD) Gap</div><img style="border:solid 1px #000000;" src="' . SITEURL . 'library/stacked.chart_jd_gen.php?' . $uri . '">';		
		$pdf->Cell ( 0, 8, ($sectionNum++).'. Student\'s effectiveness in applying the self-review diagnostic', 0, 1, 'C', true );
		$html = '<div style="page-break-inside:avoid;">'.$graph.'
				</div>';
		$pdf->SetTextColor ( 0, 0, 0 );
		$pdf->SetFont ( 'times', '', 12 );
		...
		$pdf->SetDefaultMonospacedFont ( PDF_FONT_MONOSPACED );
		// set margins
                // $pdf->SetHeaderMargin (0, PDF_MARGIN_HEADER,0 );
		//$pdf->SetMargins ( PDF_MARGIN_LEFT, PDF_MARGIN_TOP, PDF_MARGIN_RIGHT );
		...
		//if($this->aqsData['client_id']==11)
			//$firstpagehtml = '<table class="pdfHdr broad"> <tbody><tr><td class="halfSec fl" align="left"><a href=""><img src="' . SITEURL . 'public/images/logo.png"alt="" style="height:55px;"></a></td><td class="halfSec fr" align="right" ><a href=""><img src="'.SITEURL.'/public/images/shishuvan.jpg" alt=""></a></td></tr></table>';
			//<tr><td colspan="2" style="font-weight:bold;text-align:center;font-size:13px;">The Student Performance Review has been initiated and validated by '.($this->aqsData['school_name']).'</td></tr></table>';
		//else		 
			//$firstpagehtml = '<div style="text-align:center;"><div class="page-header page-cover" style="height:180px;background-color:#6c0d10;"><div style="padding-top:0px;padding-bottom:0px;padding-right:20px;height:100px" class="header-img"><img src="./public/images/reports/Rep-logo.png"></div></div></div><br/>';
		$firstpagehtml .= <<<EOD
...
EOD;
		...
		//award
                ...
                $schoolDetailsHtml .= '<table style="align:center;padding:3px;font-size:15px;" ><tr><td><b>School:</b> '.$this->aqsData['school_name'].'</td><td></td></tr>'

                        .'<tr><td><b>Principal: </b>'.$this->aqsData['principal_name'].'</td><td></td></tr>'
                        .'<tr><td><b>Review Date: </b>'.ChangeFormat($rating_date,"d F Y","").'</td><td align="center"><b>Award: </b>'.$this->awardName.'</td></tr>'
                        .'<tr><td><b>Report time period: </b>'.ChangeFormat($start_date,"d F Y","").' to '.ChangeFormat($end_date,"d F Y","").'</td><td></td></tr>'
                . '</table>';
                ...
                $kpaDetailsHtml .= '<style>th{text-align:center;}</style><table width="100%" border="1"  style="padding:5px;"><thead>'
                        .'<tr>'
                        . '<th width="25%"><b>KPA Name</b></th>'
                        . '<th width="25%"><b>Key Question</b></th>'
                        . '<th width="25%"><b>Sub Question</b></th>'
                        . '<th width="25%"><b>Judgement Statement</b></th>'
                        . '</tr></thead><tbody>';
                ...
                if(!empty($this->actionPlanKpKQ['kpa'])){
                   ...
                   // print_r($kqs);
                    foreach($kps as $key=>$val){
                        $kpaDetailsHtml .=  '<tr nobr="true"><td width="25%" align="left">'.$val.'</td>'
                                . '<td width="25%">'.$kqs[$key].'</td>'
                                . '<td width="25%">'.$cqs[$key].'</td>'
                                . '<td width="25%">'.$js[$key].'</td></tr>';
                    }
                }
                ...
                $pdf->writeHTMLCell ( 0, 0, '', '', $extRecommendationHtml, 0, 1, 0, true, 'J', true );
                   ...
                   // $pdf->writeHTMLCell ( 0, 0, '', '', $impactStatementHtml, 0, 1, 0, true, 'J', true );
                    ...
                    foreach($this->actionStatements as $key=>$data){
                        ...
                        if(!empty($this->actionPlanStatements[$data['assessor_action1_impact_id']])) {
                            ...
                            // $pdf->writeHTMLCell ( 0, 0, '', '', $impactMethodDetailsHtml, 0, 1, 0, true, 'J', true ); 
		...
		$this->config["isShishuvanTeacherReview"] = $this->aqsData['client_id']==11?1:0;
                ...
                //$this->config["isStudentReview"] =$this->studentInfo['name']['value'];
                
                $this->aqsInfo='<div class="reportInfo"><b>A compilation of scores based on<br>Self-Review and Evaluation<br>&<br>External Review and Evaluation</b></div><div class="reportInfo"><b>Conducted in:  {conductedOn}<br>Valid until: {validTill}</b></div>';
...
where a.client_id='$clId'  &&  (d.assessment_type_id=1 || d.assessment_type_id=5) && a.isAssessmentActive=1 && c.is_guest!=1 and  a.diagnostic_id = '$dgId' and a.aqs_round IN(1,2)and a.d_sub_assessment_type_id!=1 and d.assessment_type_id=1 group by a.assessment_id having 1 ) ) as z  order by assessment_round asc limit 0,2";
	...
	protected function loadAqsData(){
        ...
        protected function loadAqsDataRound21($assess_allow){
             if(empty($assessmentId)){
		...
		if(empty($this->aqsData)){
				...
				left join h_assessment_report c on b.assessment_id = c.assessment_id and c.report_id= $this->reportId
				left join d_school_region sr on sr.region_id = a.school_region_id			
				where b.assessment_id IN (".(implode(",",$assess_allow)).") 
				group by a.id;";
				...
				left join h_assessment_report c on b.assessment_id = c.assessment_id and c.report_id= $this->reportId 
                                left join d_user du on dc.client_id = du.client_id    
                                left join h_user_user_role dur on du.user_id = dur.user_id && dur.role_id=6
				where  dur.role_id=6 && b.assessment_id IN (".(implode(",",$assess_allow)).") 
				group by b.assessment_id;";
                                //$this->aqsData=$this->db->get_row($sql,array($this->assessmentId));
                               $this->aqsData=$this->db->get_results($sql);//print_r($this->aqsData); 
                        }
	...
	protected function loadJudgementalStatements($is7thKpaReport=false,$lang_id=DEFAULT_LANGUAGE,$round=1){
		if(empty($this->judgementStatement) || $round==2){
			...
			if($round == 2){
                            ...
                            $this->judgementStatement=$this->get_section_Array($this->db->get_results($sql,$params),"judgement_statement_instance_id","core_question_instance_id");
                        }
		}
	}
			...
			elseif(count($this->kpas)==1){ 
                                switch($this->reportId){
                                    ...
                                    case 1 : $kpa=current($this->kpas);
                                              ...
                                              break;
                                    case 10:      
                                    case 7 :  $kpa=current($this->kpas);//single teacher recommendation report
                                                reset($this->kpas);			
                                                $teacherAward = isset($kpa['externalRating'])?$kpa['externalRating']['score']:'';
                                                $this->awardName=$teacherAward<4?' working towards \'Proficiency\'':' awarded \'Proficient\'';
						break;
	...
	protected function loadActionPlanAward($lang_id=DEFAULT_LANGUAGE){
			...
			if(count($this->kpas)>1){
						$temp=array();
							...
							foreach($this->kpas as $kpa){
								...
								$temp[]=isset($kpa['externalRating']['score'])?$kpa['externalRating']['score']:'';
                                                                }
							}
							$compulsoryKpaScore1=$temp[0]; //We have assumes that L & T KPA are the top two KPAs. So we are hard coding it. We need to find a better way.
							$compulsoryKpaScore2=$temp[1];
	...
	protected function loadAssessorKeyNotes(){
		if(empty($this->keyNotes)){
			$sql="SELECT * FROM assessor_key_notes where rec_type=0 && assessment_id=? order by type;";
			$this->keyNotes=$this->db->array_grouping($this->db->get_results($sql,array($this->assessmentId)),"kpa_instance_id");
		}
	}
	
	protected function loadRecommendations($lang_id=DEFAULT_LANGUAGE){
		if(empty($this->recommendationText)){
			$sql="select distinct c.judgement_statement_instance_id,hlt.translation_text as recommendation_text,r.rating_id
				from f_score a
				...
				where a.isFinal = 1 and a.assessment_id = ? and hlt.language_id=?
				order by c.`js_order` asc;";
			$this->recommendationText=$this->db->array_grouping($this->db->get_results($sql,array($this->assessmentId,$lang_id)),"judgement_statement_instance_id");
		}
	}
	protected function loadActionPlanKpaData($lang=DEFAULT_LANGUAGE){
		...
		if(empty($this->actionPlanStatements)){		 
                  ...
                  /*  $sql="SELECT  am.lw,am.co,am.bl,am.intr,im.statement_id,imt.method,im.comments,cs.class_name,cs.class_comments,ids.stak_comments,ids.designation,im.date
                    ...
                    INNER JOIN (SELECT statement_id,count(if(`activity_method_id`=1,`activity_method_id`,null)) as lw,count(if(`activity_method_id`=2,`activity_method_id`,null)) as co ,count(if(`activity_method_id`=3,`activity_method_id`,null)) as bl,count(if(`activity_method_id`=4,`activity_method_id`,null)) as intr FROM `h_impact_statement` WHERE action_plan_id=?) am ON am.statement_id=im.statement_id
                    WHERE  im.action_plan_id = ?  ORDER BY im.date  " ;*/
		...
		if(empty($this->actionStatements)){		 
               $sql="SELECT d.designation,im.impact_statement,im.assessor_action1_impact_id,im.assessor_action1_id,im.designation_id FROM h_assessor_action1_impact im
                ...
                $this->actionStatements=$this->db->get_results($sql,array($this->actionPlanId));
                   ...
                   //echo "<pre>"; print_r($this->actionStatements);
		}
	}
	...
	protected function generateIndexAndCover($diagnosticLabels = array()){
		...
		if($this->reportId==9){
                $valuesToReplace=array($this->aqsData['school_name'],$this->aqsData['school_address'],$this->conductedDate,$this->validDate,$this->awardName,date("d-m-Y"),$this->schoolLocation,$this->schoolCity,$this->schoolState,$this->schoolCountry,!empty($this->studentInfo['name']['value'])?$this->studentInfo['name']['value']:'');
                    
                }else{
                $valuesToReplace=array($this->aqsData['school_name'],$this->aqsData['school_address'],$this->conductedDate,$this->validDate,$this->awardName,date("d-m-Y"),$this->schoolLocation,$this->schoolCity,$this->schoolState,$this->schoolCountry,!empty($this->teacherInfo['name']['value'])?$this->teacherInfo['name']['value']:'');
                }
	...
	protected function generateSection_TchrAwardDefintion(){
		$indexKey=$this->addIndex("Teacher Performance Review: Grade Definition");
		$section=array("sectionHeading"=>array("text"=>"Teacher Performance Review: Grade Definition"),"sectionBody"=>array(),'indexKey'=>$indexKey);						
		$block=array(
				"blockHeading"=>array(
		...
		/*$block=array(
                ...
                $block=array("blockBody"=>array(
						"dataArray"=>array(
								array('<div style="width:100%"><img src="'.$this->config['StudentAwardDefi'].'" style="width:100%"></div>')
						)));
		$section['sectionBody'][]=$block;
		//$this->sectionArray[]=$section;
		//$indexKey=$this->addIndex("Key for reading the report");		
	...
	protected function generateSection_ScoreCardForKPAs($skipComparisonSection=0,$lang_id,$diagnosticLabels=array()){
		...
		foreach($this->kpas as $kpa){
			$kpa_count++;
			if($isCollobrative==1){
                        $kpaValuesForGraph[]=array("values"=>array(isset($kpa['internalRating'])?$kpa['internalRating']['score']:0),"name"=>$kpa['KPA_name']);
							...
							"blockBody"=>array(
								"dataArray"=>array(
									array(
										...
										'<div class="pull-left">Collaborative-Review Rating (CSRE)</div><div class="pull-left"><span class="'.(isset($kpa['internalRating'])?"score-".$kpa['internalRating']['score']:"").'">'.(isset($kpa['internalRating'])?$kpa['internalRating']['rating']:"")."</span></div>")
								)
							),
			...
			if(isset($this->keyQuestions[$kpa['kpa_instance_id']])){
				...
				foreach($this->keyQuestions[$kpa['kpa_instance_id']] as $keyQ){
					...
					$cqBlock=array(
								"blockBody"=>array(
									"dataArray"=>array(
										array("<span class='nobr'>Collaborative-Review Grade for S.Q.</span>")
					...
					$kqBlock=array(
								"blockBody"=>array(
									"dataArray"=>array(
										array("<span  class='nobr'>Collaborative-Review Grade for K.Q.</span>"),
		...
		if($skipComparisonSection==0 && $this->config['getCreateNetType']!=1){
         ...
         protected function generateSection_KeyQuestionRound2($skipComparisonSection=0,$lang_id,$diagnosticLabels=array(),$i,$isCollobrativeR1,$isCollobrativeR2,$ids_allow = array()){
		...
		$comparisonSection=array();
               ...
               // echo "<pre>";print_r($this->keyQuestions);die;
                if(!empty($this->assessmentIdRound2) && !empty($this->assessmentId)) {
                   ...
                   // echo "<pre>";print_r($this->kpas[0]);die;
                   ...
                   // echo "<pre>";print_r($twoRoundJudgemntStmnt);die;
                    ...
                    if(!empty($twoRoundCoreQuestion[$this->assessmentId]) && !empty($twoRoundCoreQuestion[$this->assessmentIdRound2])){
                    ...
                    if(!empty($twoRoundJudgemntStmnt[$this->assessmentId]) && !empty($twoRoundJudgemntStmnt[$this->assessmentIdRound2])){
         ...
         protected function createComparisionPieChart($sameRatingKey,$improvedRatingKey,$decreasedRatingKey,$file_name){
                ...
                $character = '{"options":'
                        ...
                        . '"legend": {"fontSize": "20px","fontWeight":"bold","align":"right","layout":"vertical","verticalAlign":"top","x": 15,"y": 110},'
                                        ...
                                        $kpaBlock=array(
                                                                "blockBody"=>array(
                                                                        "dataArray"=>array(
                                                                                array(
                                                                                      $kpa['KPA_name']." ".$diagnosticLabels['assessment_for'],
										'<div class="pull-left">Collaborative-Review Rating Round1<br>'.$diagnosticLabels['external_review_rating'].' Round2</div><div class="pull-left"><span class="'.(isset($kpa['externalRating'])?"score-".$kpa['externalRating']['score']:"").'">'.(isset($kpa['externalRating'])?$kpa['externalRating']['rating']:"").'</span><br/>'.($this->subAssessmentType==1?'<span>N/A</span>':'<span class="'.(isset($kpa['ExternalR2'])?"score-".$kpa['ExternalR2']['score']:"").'">'.(isset($kpa['ExternalR2'])?$kpa['ExternalR2']['rating']:"").' '.$rating_img_kpa.'</span>')."</div>")
                                        ...
                                        $kpaBlock=array(
                                                                "blockBody"=>array(
                                                                        "dataArray"=>array(
                                                                                array(
                                                                                      $kpa['KPA_name']." ".$diagnosticLabels['assessment_for'],
										'<div class="pull-left">'.$diagnosticLabels['external_review_rating'].' Round1<br>Collaborative-Review Rating Round2</div><div class="pull-left"><span class="'.(isset($kpa['externalRating'])?"score-".$kpa['externalRating']['score']:"").'">'.(isset($kpa['externalRating'])?$kpa['externalRating']['rating']:"").'</span><br/>'.($this->subAssessmentType==1?'<span>N/A</span>':'<span class="'.(isset($kpa['ExternalR2'])?"score-".$kpa['ExternalR2']['score']:"").'">'.(isset($kpa['ExternalR2'])?$kpa['ExternalR2']['rating']:"").' '.$rating_img_kpa.'</span>')."</div>")
                                        ...
                                        $kpaBlock=array(
                                                                "blockBody"=>array(
                                                                        "dataArray"=>array(
                                                                                array(
                                                                                      $kpa['KPA_name']." ".$diagnosticLabels['assessment_for'],
										'<div class="pull-left">Collaborative-Review Rating Round1<br>Collaborative-Review Rating Round2</div><div class="pull-left"><span class="'.(isset($kpa['externalRating'])?"score-".$kpa['externalRating']['score']:"").'">'.(isset($kpa['externalRating'])?$kpa['externalRating']['rating']:"").'</span><br/>'.($this->subAssessmentType==1?'<span>N/A</span>':'<span class="'.(isset($kpa['ExternalR2'])?"score-".$kpa['ExternalR2']['score']:"").'">'.(isset($kpa['ExternalR2'])?$kpa['ExternalR2']['rating']:"").'</span>').' '.$rating_img_kpa."</div>")
				...
				$kpaBlock=array(
							"blockBody"=>array(
								"dataArray"=>array(
									array(
										 $kpa['KPA_name']." ".$diagnosticLabels['assessment_for'],
										'<div class="pull-left">'.$diagnosticLabels['external_review_rating'].' Round1<br>'.$diagnosticLabels['external_review_rating'].' Round2</div><div class="pull-left"><span class="'.(isset($kpa['externalRating'])?"score-".$kpa['externalRating']['score']:"").'">'.(isset($kpa['externalRating'])?$kpa['externalRating']['rating']:"").'</span><br/>'.($this->subAssessmentType==1?'<span>N/A</span>':'<span class="'.(isset($kpa['ExternalR2'])?"score-".$kpa['ExternalR2']['score']:"").'">'.(isset($kpa['ExternalR2'])?$kpa['ExternalR2']['rating']:"").' '.$rating_img_kpa.'</span>')."</div>")
								)
							),
							"style"=>"bordered kpablock"
						);
			...
			if(isset($this->keyQuestionsR[0][$kpa['kpa_instance_id']])){
				...
				foreach($this->keyQuestionsR[0][$kpa['kpa_instance_id']] as $keyQ){
					...
					$cqBlock=array(
								"blockBody"=>array(
									"dataArray"=>array(
										array("<span class='nobr'>Collaborative-Review Grade for S.Q. R1</span>"),
								...
								"blockBody"=>array(
									"dataArray"=>array(
										...
										array("<span class='nobr'>Collaborative-Review Grade for S.Q. R2</span>")
					...
					$kqBlock=array(
								...
								"blockBody"=>array(
									"dataArray"=>array(
										...
										array("<span  class='nobr'>Collaborative-Review Grade for K.Q. R2</span>")
									)
								),
								"style"=>"bordered kpaStyle",
								"config"=>array('groupby'=>"kpa-".$keyQ['key_question_instance_id'])
					...
					if(isset($this->coreQuestionsR[0][$keyQ['key_question_instance_id']])){
						foreach($this->coreQuestionsR[0][$keyQ['key_question_instance_id']] as $coreQ){
							...
							foreach($this->judgementStatement[0][$coreQ['core_question_instance_id']] as $statment){
								...
								$jsBlock['blockBody']['dataArray'][4][]=array($rating_img_js);
                                                                //}
		...
		if($skipComparisonSection==0 && $this->config['getCreateNetType']!=1){
			...
			$graphBlock=array(
						...
						"blockBody"=>array(
							"dataArray"=>array(
								...
								array($this->getGraphHTML($kpaValuesForGraph,$bar_keys,4,1,array("CSRE R1","SERE R2"),isset($diagnosticLabels['KPA_FULLFORM'])?$diagnosticLabels['KPA_FULLFORM']:"Key Performance Areas (KPAs)","Grades"))
							)
						),
					"style"=>"bordered barGraph"
				);
			...
			$graphBlock=array(
						...
						"blockBody"=>array(
							"dataArray"=>array(
								...
								array($this->getGraphHTML($kpaValuesForGraph,$bar_keys,4,1,array("SERE R1","CSRE R2"),isset($diagnosticLabels['KPA_FULLFORM'])?$diagnosticLabels['KPA_FULLFORM']:"Key Performance Areas (KPAs)","Grades"))
							)
						),
					"style"=>"bordered barGraph"
				);
			...
			$graphBlock=array(
						...
						"blockBody"=>array(
							"dataArray"=>array(
								...
								array($this->getGraphHTML($kpaValuesForGraph,$bar_keys,4,1,array("CSRE R1","CSRE R2"),isset($diagnosticLabels['KPA_FULLFORM'])?$diagnosticLabels['KPA_FULLFORM']:"Key Performance Areas (KPAs)","Grades"))
							)
						),
					"style"=>"bordered barGraph"
				);
			...
			$graphBlock=array(
						...
						"blockBody"=>array(
							"dataArray"=>array(
								...
								array($this->getGraphHTML($kpaValuesForGraph,$bar_keys,4,1,array("SERE R1","SERE R2"),isset($diagnosticLabels['KPA_FULLFORM'])?$diagnosticLabels['KPA_FULLFORM']:"Key Performance Areas (KPAs)","Grades"))
							)
						),
					"style"=>"bordered barGraph"
				);
			...
			$comparisonSection['sectionBody'][]=$keysBodyBlock;
                       ...
                       // echo "<pre>";print_r($comparisonSection);
			$this->sectionArray=array_merge($this->sectionArray,array($comparisonSection));
                 ...
                 //get pi chart 
					...
					"style"=>"bordered barGraph"
                       ...
                       // echo "<pre>";print_r($piChartBlock);die;
                        $graphSection['sectionBody'][]=$piChartBlock;
                       // $this->sectionArray=array_merge($this->sectionArray,array($comparisonSection));
	...
	protected function getGraphHTML($valuesArray,$steps,$maxValue,$minValue=0,$barDes=array(),$infoBelowGraph="",$infoOnYAxis=""){
		...
		$totalHeight=$topBarHeight+$bottomBarHeight+$graphHeight;*/
                ...
                if(!empty($s3_upload_url_key)){
                    ...
                    $html .='<tr bgcolor="orange"><td colspan="3">At Key Question Level:</td></tr>';
                    $html .= '<tr><td width="45%" align="left" style="font-size: 11px;font-weight: bold;font-family:Helvetica,Arial;line-height:1.7;">'
                           . '<table width="100%"><tr><td width="25%">K.Q. with Improved Ratings:'.$this->improvedRatingKey.'</td></tr><tr><td></td></tr>'
                            . '<tr><td width="100%">K.Q. with Same Ratings:'.$this->sameRatingKey.'</td></tr><tr><td></td></tr>'
                            . '<tr><td width="100%">K.Q. with Decreased Ratings:'.$this->decreasedRatingKey.'</td></tr></table></td>'
                            . '<td width="30%" align="left"><img style="height:300px;width:430px;" src="'.$s3_upload_url_key.'"></td>'
                            . '<td width="25%" align="left" style="font-size: 11px;font-weight: bold;font-family:Helvetica,Arial;line-height:1.7;">'
                            . '<table width="100%"><tr><td width="25%"><img height="10px" width="10px" src=./public/images/green3.png>  Improved Ratings</td></tr><tr><td></td></tr>'
                            . '<tr><td width="100%"><img height="10px" width="10px" src=./public/images/blue2.png>  Same Ratings</td></tr><tr><td></td></tr>'
                            . '<tr><td width="100%"><img height="10px" width="10px" src=./public/images/red1.png>  Decreased Ratings</td></tr></table>'
                            . '</td></tr>';
                }
                ...
                if(!empty($s3_upload_url_core)){
                    ...
                    $html .='<tr bgcolor="orange"><td colspan="3">At Sub Question Level:</td></tr>';
                    $html .= '<tr><td width="45%" align="left" style="font-size: 11px;font-weight: bold;font-family:Helvetica,Arial;line-height:1.7;">'
                            . '<table width="100%"><tr><td width="25%">S.Q. with Improved Ratings:'.$this->improvedRatingCore.'</td></tr><tr><td></td></tr>'
                            . '<tr><td width="100%">S.Q. with Same Ratings:'.$this->sameRatingCore.'</td></tr><tr><td></td></tr>'
                            . '<tr><td width="100%">S.Q. with Decreased Ratings:'.$this->decreasedRatingCore.'</td></tr></table></td>'
                            . '<td width="30%" align="left"><img style="height:300px;width:430px;" src="'.$s3_upload_url_core.'"></td>'
                            . '<td width="25%" align="left" style="font-size: 11px;font-weight: bold;font-family:Helvetica,Arial;line-height:1.7;">'
                            . '<table width="100%"><tr><td width="25%"><img height="10px" width="10px" src=./public/images/green3.png>  Improved Ratings</td></tr><tr><td></td></tr>'
                            . '<tr><td width="100%"><img height="10px" width="10px" src=./public/images/blue2.png>  Same Ratings</td></tr><tr><td></td></tr>'
                            . '<tr><td width="100%"><img height="10px" width="10px" src=./public/images/red1.png>  Decreased Ratings</td></tr></table>'
                ...
                if(!empty($s3_upload_url_js)){
                    ...
                    $html ='<tr bgcolor="orange"><td colspan="3">At Judgement Statement Level:</td></tr>';
                    $html .= '<tr><td width="45%" align="left" style="font-size: 11px;font-weight: bold;font-family:Helvetica,Arial;line-height:1.7;">'
                            . '<table width="100%"><tr><td width="25%" style="font-size: 11px;font-weight: bold;font-family:Helvetica,Arial;">J.S. with Improved Ratings:'.$this->improvedRatingSt.'</td></tr><tr><td></td></tr>'
                            . '<tr><td width="100%" style="font-size: 11px;font-weight: bold;font-family:Helvetica,Arial;">J.S. with Same Ratings:'.$this->sameRatingSt.'</td></tr><tr><td></td></tr>'
                            . '<tr><td width="100%" style="font-size: 11px;font-weight: bold;font-family:Helvetica,Arial;">J.S. with Decreased Ratings:'.$this->decreasedRatingSt.'</td></tr></table></td>'
                            . '<td width="30%" align="left"><img style="height:300px;width:430px;" src="'.$s3_upload_url_js.'"></td>'
                            . '<td width="25%" align="left" style="font-size: 11px;font-weight: bold;font-family:Helvetica,Arial;line-height:1.7;">'
                            . '<table width="100%"><tr><td width="25%" style="font-size: 11px;font-weight: bold;font-family:Helvetica,Arial;"><img height="10px" width="10px" src=./public/images/green3.png>  Improved Ratings</td></tr><tr><td></td></tr>'
                            . '<tr><td width="100%" style="font-size: 11px;font-weight: bold;font-family:Helvetica,Arial;"><img height="10px" width="10px" src=./public/images/blue2.png>  Same Ratings</td></tr><tr><td></td></tr>'
                            . '<tr><td width="100%" style="font-size: 11px;font-weight: bold;font-family:Helvetica,Arial;"><img height="10px" width="10px" src=./public/images/red1.png>  Decreased Ratings</td></tr></table>'
	...
	protected function recommendationsOnKpa($lang_id=DEFAULT_LANGUAGE,$diagnosticLabels=array()){
                $isCollobrative=$this->config['isCollobrative'];        
		$kpa_count=0;
		foreach($this->kpas as $kpa){
			$kpa_count++;			
			...
			if(isset($this->keyQuestions[$kpa['kpa_instance_id']])){
				...
				foreach($this->keyQuestions[$kpa['kpa_instance_id']] as $keyQ){
					...
					$keyQBlock=array(
						"blockBody"=>array(
							"dataArray"=>array(
								array(isset($diagnosticLabels['Key_Question'])?$diagnosticLabels['Key_Question']:"Key Question $keyQ_count",array("text"=>"&nbsp;","rSpan"=>6),array("text"=>$keyQ['key_question_text'],"cSpan"=> isset($this->coreQuestions[$keyQ['key_question_instance_id']])?count($this->coreQuestions[$keyQ['key_question_instance_id']]):0 ,"style"=>(isset($keyQ['externalRating'])?"score-bg-".$keyQ['externalRating']['score']." scoreB-".$keyQ['externalRating']['score']:""))),
								array(isset($diagnosticLabels['Sub_Question'])?$diagnosticLabels['Sub_Question']:'Sub Questions'),
								array(array("text"=>isset($diagnosticLabels['Outstanding'])?$diagnosticLabels['Outstanding']:"Outstanding","style"=>"score-4")),
								array(array("text"=>isset($diagnosticLabels['Good'])?$diagnosticLabels['Good']:"Good","style"=>"score-3")),
								array(array("text"=>isset($diagnosticLabels['Variable'])?$diagnosticLabels['Variable']:"Variable","style"=>"score-2")),
								array(array("text"=>isset($diagnosticLabels['Needs_Action'])?$diagnosticLabels['Needs_Action']:"Needs Action","style"=>"score-1"))
							)
						),
						"style"=>"keyQblock",
					...
					if(isset($this->coreQuestions[$keyQ['key_question_instance_id']])){
						foreach($this->coreQuestions[$keyQ['key_question_instance_id']] as $coreQ){
							...
							$coreQsInKPA++;
                                                        if($isCollobrative==1){
                                                        $keyQBlock['blockBody']['dataArray'][1][]=array("text"=>'<span class="min-h">'.$coreQsInKPA.". ".$coreQ['core_question_text'].'</span>',"style"=>(isset($coreQ['internalRating'])?"scoreB-".$coreQ['internalRating']['score']:""));
							...
							foreach($this->judgementStatement[$coreQ['core_question_instance_id']] as $statment){
								$satatement_count++;
                                                                ...
                                                                if($isCollobrative==1){
                                                                 if(isset($statment['internalRating'])){
									$values[$statment['internalRating']['score']][]=$coreQsInKPA.$numberToAlpha[$satatement_count];
								}   
                                                                }else{
		...
		foreach($this->kpas as $kpa){
			...
			if(isset($this->keyQuestions[$kpa['kpa_instance_id']])){
				...
				for($i=0;$i<=$url_count;$i++){
                                  //echo  $url[$i];
					$graph .= '<div style="page-break-inside:avoid;"><div style="text-align:center;font-weight:bold;"><span style="font-weight:normal;">Table '.$this->tableNum.'</span><br/> Rating performance data at KPA level  ratings arranged from
highest to lowest accuracy</div><img style="border:solid 1px #000000;" src="' . SITEURL . 'library/stacked.chart_jr.php' . $url[$i] . '&lang_id='.$lang_id.'" /></div>';													
				}
                                $comparisonSection = array();
        ...
        SERE, which is often indicative of the level of experience between the two teams regarding global standards.")
						)
					),
				"style"=>"textBlock"
			);
			...
			if(isset($this->keyQuestions[$kpa['kpa_instance_id']])){
				...
				for($i=0;$i<=$url_count;$i++){
					...
					$graph .= '<div style="page-break-inside:avoid;"><div style="text-align:center;font-weight:bold;"><span style="font-weight:normal;">Table '.$this->tableNum.'</span>
                                            <br/>Judgement distance between the self-review and external review ratings arranged from highest to lowest accuracy</div><img style="border:solid 1px #000000;" src="' . SITEURL . 'library/stacked.chart_jd.php' . $url[$i] . '&jd=1&lang_id='.$lang_id.'" /></div>';													
				}
                                $comparisonSection = array();
						...
						foreach($this->coreQuestions[$keyQ['key_question_instance_id']] as $coreQ){
							...
							foreach($this->judgementStatement[$coreQ['core_question_instance_id']] as $statment){
								...
								$satatement_count++;
                                                                if($isCollobrative==1){
                                                                  if(isset($statment['internalRating'])){
                                                                    if(isset($statment['internalRating']['score']) && $statment['internalRating']['score'] == '3'){
                                                                        $jsm++;
                                                                    }else if(isset($statment['internalRating']['score']) && $statment['internalRating']['score'] == '4'){
                                                                        $jsa++;
                                                                    }else if(isset($statment['internalRating']['score']) && $statment['internalRating']['score'] == '2'){
                                                                        $jss++;
                                                                    }else if(isset($statment['internalRating']['score']) && $statment['internalRating']['score'] == '1'){
                                                                        $jsr++;
                                                                    }
									$values[$statment['internalRating']['score']][]=$coreQsInKPA.$numberToAlpha[$satatement_count];
								}  
...
highest to lowest</div><img style="border:solid 0px #000000;" src="data:image/png;base64,'.base64_encode($result).'" /></div>';
	...
	protected function schoolEffectivenessInApplyingSelfReview($diagnosticLabels=array()){
                $isCollobrative=$this->config['isCollobrative']; 
                if($isCollobrative==1){
                $totkap=count($this->kpas);    
                $indexKey=$this->addIndex("Performance across ".$totkap." Key Performance Areas");
		$section=array("sectionHeading"=>array("text"=>"Performance across ".$totkap." Key Performance Areas"),"sectionBody"=>array(),'indexKey'=>$indexKey);
		...
		$section=array("sectionHeading"=>array("text"=>"School's effectiveness in applying the self-review diagnostic"),"sectionBody"=>array(),'indexKey'=>$indexKey);
                ...
                if($isCollobrative==1){
                 $kpaComparisonBlock=array(
								"blockHeading"=>array(
									"data"=>array("Key Performance Area (KPA)","CSRE Judgements (Collaborative Review)")
								),
								"blockBody"=>array(
									"dataArray"=>array()
								),
		...
		if($isCollobrative==1){
                   $textBlock=array(
					"blockBody"=>array(
						"dataArray"=>array(
							array("When the numerical scores for the collaborative review results were applied to the Adhyayan Quality Standard rubric, the school was identified as achieving $this->awardName status."),
							array("The following graph represents the ratings of the CSRE team.")
						)
					),
				"style"=>"textBlock"
			); 
		...
		$section['sectionBody'][]=$textBlock;
                ...
                if($isCollobrative==1){
                  $graphBlock=array(
					"blockHeading"=>array(
						"data"=>array("Bar graph representation of above ratings")
					),
					"blockBody"=>array(
						"dataArray"=>array(
							array($this->getGraphHTML($kpaValuesForGraph,array(4=>isset($diagnosticLabels['Outstanding'])?$diagnosticLabels['Outstanding']:"Outstanding",3=>isset($diagnosticLabels['Good'])?$diagnosticLabels['Good']:"Good",2=>isset($diagnosticLabels['Variable'])?$diagnosticLabels['Variable']:"Variable",1=>isset($diagnosticLabels['Needs_Attention'])?$diagnosticLabels['Needs_Attention']:"Needs Attention"),4,1,$isCollobrative==1?array("CSRE Rating"):array("SSRE","SERE"),isset($diagnosticLabels['KPA_FULLFORM'])?$diagnosticLabels['KPA_FULLFORM']:"Key Performance Areas (KPAs)","Grades"))
						)
					),
				"style"=>"bordered barGraph"
			);  
                  ...
                  $textBlock=array(
					"blockBody"=>array(
						"dataArray"=>array(
							...
							array('Adhyayan\'s experience is that schools that choose to review themselves are keen to give students the most successful education experience possible. They desire and strive to constantly improve and embrace challenge and change. In preparing to set school improvement targets it will be important forthe school to study the diagnostic carefully. Some exemplars can be found on our website <a target="_blank" href="http://www.adhyayan.asia/" style="text-decoration:underline;">www.adhyayan.asia</a> in our \'Gallery\' of what good looks like.')
						)
					),
				"style"=>"textBlock"
			);  
	...
	protected function assessorKeyNotes(){
		if(count($this->keyNotes)==0)
			return;
		$indexKey=$this->addIndex("Assessor Notes on Strengths and Recommendations");
		$section=array("sectionHeading"=>array("text"=>"Assessor Notes on Strengths and Recommendations across ".count($this->kpas)." Key Performance Areas (KPAs) "),"sectionBody"=>array(),'indexKey'=>$indexKey);
		...
		foreach($this->kpas as $kpa){
			if(isset($this->keyNotes[$kpa['kpa_instance_id']])){
				...
				foreach($this->keyNotes[$kpa['kpa_instance_id']] as $kn){
					$kn_count++;
					$block['blockBody']['dataArray'][]=array($kn_count.".",(!empty($kn['type'])?ucfirst($kn['type']).': ':'').$kn['text_data']);
				}
				$section['sectionBody'][]=$block;
			}
		}
	...
	protected function adhayayanQualityStandard(){
		...
		if($isCollobrative==1){
				...
				"blockBody"=>array(
					"dataArray"=>array(
						array('Following a collaborative review undertaken by the school team under the guidance of Adhyayan assessors, we are delighted to award <b>'.$this->aqsData['school_name'].'</b> with the Adhyayan Quality Standard <b>'.$this->awardName.'</b>. Congratulations!<br><br>'),
						array('This award is valid until '.$this->validDate.'. The expectation is that the school will celebrate it\'s areas of strength and address its challenges. Targeted school improvement will require the school to take action on the recommendations provided by Adhyayan in this report. The improvement journey will be documented each year, as the school  undertakes its annual self-review on its own or supported by Adhyayan.'),
					)
				),
				"style"=>"textBlock"
			);    
		...
		$block=array(
				...
				"blockBody"=>array(
					"dataArray"=>array(
             		...
             		<li  >During the review of the school, its Collaborative School Review & Evaluation (CSRE) Team members were trained on how to use Adhyayan\'s School Review Diagnostic. The team members were informed of the importance of making collective decisions based on substantiated, clearly available and visible evidence.</i></li><br>
             		<li>The CSRE Team was divided into three teams with each team guided by an Adhyayan assessor. Each team undertook separate reviews focusing on two Key Performance Areas (KPAs) each. Their judgements were then distilled into a single judgement of the school\'s performance against each of the Adhyayan Key Performance Areas (KPAs).</li><br>
             		<li>The CSRE Team then awarded the school its Adhyayan Quality Standard Award.</li><br>
             		<li>The CSRE team members:<br><br>
             			<ol style="list-style-type: circle;margin-left: 15px;padding:0;">
             				<li>Shared their judgements on each of the KPAs and the evidence on which they were based</li><br>
             				<li>Identified key areas for celebration and improvement</li><br>
             				<li>Began planning for improvement based on the collaborative review findings</li>	
             			</ol>
             		</li><br>
                        <li>The following report provides a confidential summary of the achievements and areas for development identified through the AQS process. It contains recommendations for school improvement within each of the KPAs.</li>
             	</ol>'),
	...
	protected function qualityStandardDefinitionBlock_schemeDonBosco(){
		...
		if(count($this->awardSchemes)==10){
			$block['blockBody']['dataArray'][]=array("International",$this->awardSchemes[3]['award_name'],$this->awardSchemes[2]['award_name'],$this->awardSchemes[1]['award_name']);
			$block['blockBody']['dataArray'][]=array("National",$this->awardSchemes[6]['award_name'],$this->awardSchemes[5]['award_name'],$this->awardSchemes[4]['award_name']);
			$block['blockBody']['dataArray'][]=array(array("text"=>"State","rSpan"=>2),$this->awardSchemes[9]['award_name'],$this->awardSchemes[8]['award_name'],$this->awardSchemes[7]['award_name']);
			$block['blockBody']['dataArray'][]=array(array("text"=>$this->awardSchemes[10]['award_name'],"cSpan"=>3,"style"=>"text-center"));
		}
		return $block;
	}
	...
	protected function qualityStandardDefinitionBlock_standard(){
		...
		$block=array(
				...
				"blockBody"=>array(
					"dataArray"=>array(
						array('<img  src="'.SITEURL.'public/images/reports/platinum.png" height="40" width="40"/>','<b>Platinum:</b> The Platinum award confirms that the school\'s performance is outstanding on most KPAs as measured against their chosen tier.'),
						array('<img src="'.SITEURL.'public/images/reports/gold.png" height="40" width="40"/>','<b>Gold:</b>Gold is the bedrock of the Adhyayan Quality Standard. The Gold award confirms that the school\'s performance is good and strong on most KPAs.'),
						array('<img  src="'.SITEURL.'public/images/reports/silver.png" height="40" width="40"/>','<b>Silver:</b> The Silver award indicates that the school\'s practice is variable. The Silver award confirms that while some aspects of the school\'s performance may be good, others may be satisfactory or less.'),
						array('<img  src="'.SITEURL.'public/images/reports/bronze.png" height="40" width="40"/>','<b>Bronze:</b> The Bronze award is an entry grade for schools aspiring to become high performing. While the school confirms that certain aspects of its performance within the KPAs are at least satisfactory, it is working towards greater consistency.')
					)
				),
	...
	protected function aSchoolImplovementJourney(){
		...
		$indexKey=$this->addIndex("A School Improvement Journey");
                if($isCollobrative==1){
                    $heading="The process of coming to judgement was effective in establishing:<br><br>";
                }else{
                    $heading="The <strong>Quality Dialogue</strong> was effective in establishing:<br><br>";
                }
...
And it is not enough to implement them correctly; they have to be constantly reviewed and adjusted over time as we see what works and what doesn\'t."',"style"=>"italic text-center")),
				...
				foreach($this->keyQuestions[$kpa['kpa_instance_id']] as $keyQ){
					...
					$block=array(
						...
						"blockBody"=>array(
							"dataArray"=>array(
								array(
									'<svg height="180" viewBox="0 0 100 100"  preserveAspectRatio="none" width="260px">
										<line x1="5" y1="5" x2="5" y2="95" style="stroke:#000000;stroke-width:0.2" />
										<line x1="5" y1="95" x2="95" y2="95" style="stroke:#000000;stroke-width:0.2" />
										
										<line x1="5" y1="75" x2="95" y2="75" style="stroke:#000000;stroke-width:0.2" />
										<line x1="5" y1="55" x2="95" y2="55" style="stroke:#000000;stroke-width:0.2" />
										<line x1="5" y1="35" x2="95" y2="35" style="stroke:#000000;stroke-width:0.2" />
										<line x1="5" y1="15" x2="95" y2="15" style="stroke:#000000;stroke-width:0.2" />
									...
									<div class="lcLabels">
										<div class="lcLabel">
											<div class="lcLine" style="background-color:red;"></div>
											<div class="lcText">KQ'.$kq_cnt.'SSRE</div>
										</div>
										<div class="lcLabel">
											<div class="lcLine" style="background-color:#7C7EB3;"></div>
	...
	function generateSection_TchrRecomm(){
		...
		$ImproveBlock = "";			
			...
			$block['blockBody']['dataArray'][] = array(array("text"=>$this->teacherInfo['name']['value'],"rSpan"=>1),array("text"=>$celebrateBlock.$ImproveBlock,"style"=>'assessornoteTbl'));
